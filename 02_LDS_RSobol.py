...
param_filename = f'result/02_LDS_RSobol_seed/{param_func}_run{param_run}_fp'

param_ndims_lst = [2,4,6,8][::-1]
param_nitn = 15
param_nitn_pre = 0
param_neval = int(2**23)
...
class LDSInteg():
    def __init__(self, ndims, neval, vmin, vmax, random_seed):
        ...
        # self.sampler = spt.qmc.Sobol(ndims, scramble=True)
    ...
    def set_func(self, func):
        ...
        self.func = func
        self.run_one_integration(0)
        self.set_seed(self.random_seed)
        # logger.info('LDSInteg :: set_func (end tracing)')

    def generate_samples(self, log2_n, i):
        sampler = spt.qmc.Sobol(ndims, scramble=True, seed=i)
        samples_normed = sampler.random_base2(log2_n)
        # self.sampler.reset()
        samples_normed = tf.convert_to_tensor(samples_normed, dtype=DTYPE)
        return self.vmin + (self.vmax - self.vmin) * samples_normed

    def run_one_integration(self, i):
        # print('LDSInteg :: run_one_integration (tracing)')
        # means, sdevs = [], []
        samples = self.generate_samples(int(math.log(self.neval,2)), i)
...
if __name__ == '__main__':
    ...
    for ndims in param_ndims_lst:
        ...
        for i in tqdm(range(param_nitn_pre)):
            lds_integ.run_one_integration(i)
        ...
        for i in tqdm(range(nitn_true)):
            ...
            #     break
            temp_time = time.time()
            temp_mean, temp_sdev = lds_integ.run_one_integration(i)
